...
def calcular_power_rating(df, t_casa, t_fora):
    """Motor de cálculo de xG com regularização forte"""
    ...
    min_jogos = 5
    peso_reg = 0.40  # Regressão à média conservadora
    ...
    xg_c = max(0.7, min(xg_c, 2.8))
    xg_f = max(0.7, min(xg_f, 2.8))
...
if not df.empty:
    ...
    # ===================== MODO FULL AUTO =====================
        ...
        if dados_api:
            ...
            odd_H = odd_D = odd_A = odd_O25 = odd_BTTS = 1.01

            for bookmaker in jogo_dados.get('bookmakers', []):
                for market in bookmaker.get('markets', []):
                    if market['key'] == 'h2h':
                        for outcome in market['outcomes']:
                            if outcome['name'] == jogo_dados['home_team']:
                                odd_H = max(odd_H, float(outcome['price']))
                            elif outcome['name'] == 'Draw':
                                odd_D = max(odd_D, float(outcome['price']))
                            elif outcome['name'] == jogo_dados['away_team']:
                                odd_A = max(odd_A, float(outcome['price']))
                        ...
                        for outcome in market['outcomes']:
                            if outcome['name'] == 'Over':
                                ...
                                if point_val is not None:
                                    try:
                                        if float(point_val) == 2.5:
                                            odd_O25 = max(odd_O25, float(outcome['price']))
                                    except ValueError:
                                        pass

            # Cálculo de odds compostas
            ...
            st.info("📡 Line Shopping concluído: Cotações maximizadas puxadas ao vivo.")
            ...
            t_casa = c1.selectbox("🏠 Mandante (CSV)", times_csv, 
                                index=times_csv.index(guess_h) if guess_h in times_csv else 0, 
                                key="sel_casa_auto")
            t_fora = c2.selectbox("✈️ Visitante (CSV)", times_csv, 
                                index=times_csv.index(guess_a) if guess_a in times_csv else 1, 
                                key="sel_fora_auto")
    ...
    if 'pronto_para_calcular' in locals() and pronto_para_calcular:
        ...
        if confianca <= 50:
            st.markdown(f"# 🔴 CONFIANÇA: {confianca:.0f}%")
        elif confianca <= 85:
            st.markdown(f"# 🟡 CONFIANÇA: {confianca:.0f}%")
        else:
            st.markdown(f"# 🟢 CONFIANÇA: {confianca:.0f}%")
        ...
        # Probabilidades brutas no debug
        ...
        if sugestoes_verde:
            st.success("SUGESTÕES DE OPERAÇÃO:\n\n" + "\n\n".join(sugestoes_verde))
        if sugestoes_vermelho:
            st.error("NÃO OPERAR (Risco elevado ou sem valor):\n\n" + "\n\n".join(sugestoes_vermelho))
