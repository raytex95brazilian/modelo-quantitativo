...
storage.registrar_evento_lote(secrets, tipo_evento="UPSERT", jogo=first, interface_version="V28.1.5.9")
storage.registrar_evento_lote(secrets, tipo_evento="UPSERT", jogo=second, interface_version="V28.1.5.9")
...
storage.registrar_evento_lote(secrets, tipo_evento="UPSERT", jogo=updated, interface_version="V28.1.5.9")
restored = storage.carregar_lote_pendente(secrets)
assert len(restored["Jogos"]) == 2
assert restored["Jogos"][0]["Odd mandante"] == 2.25

storage.registrar_evento_lote(secrets, tipo_evento="DELETE", jogo=first, interface_version="V28.1.5.9")
restored = storage.carregar_lote_pendente(secrets)
assert [item["ID"] for item in restored["Jogos"]] == ["g-2"]

storage.registrar_evento_lote(secrets, tipo_evento="CLEAR", interface_version="V28.1.5.9")
restored = storage.carregar_lote_pendente(secrets)
...
print("TESTE DE PERSISTÊNCIA DURÁVEL APPEND-ONLY V28.1.5.9: OK")
